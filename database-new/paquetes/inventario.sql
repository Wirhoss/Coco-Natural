...
create or replace package body pkg_inventario as
   ...
   procedure registrar_entrada_inventario (
      p_id_producto  in producto.id_producto%type,
      p_cantidad     in number,
      p_id_proveedor in proveedor.id_proveedor%type, -- opcional si no lo usas aquí
      p_usuario      in movimientos.usuario_creacion%type default user
   ) is
   begin
    -- registra el movimiento (requiere usuario_creacion)
   ...
   procedure generar_alerta_stock is
      v_usuario alertas_stock.usuario_creacion%type := user;
   begin
      for prod in (
         select id_producto,
                nombre,
