...
create or replace package pkg_proveedor as
  ...
  procedure actualizar_proveedor(p_id_proveedor in proveedor.id_proveedor%type,
                                p_nombre      in proveedor.nombre%type,
                                p_telefono    in proveedor.telefono%type,
                                p_email       in proveedor.email%type,
                                p_direccion   in proveedor.direccion%type);
  procedure eliminar_proveedor(p_id_proveedor in proveedor.id_proveedor%type);
  procedure obtener_proveedores(p_cursor out sys_refcursor);

  function cur_productos_proveedor(p_id_proveedor in proveedor.id_proveedor%type) return sys_refcursor;
...
create or replace package body pkg_proveedor as
  procedure insertar_proveedor(p_id_proveedor in proveedor.id_proveedor%type,
                               ...
                               p_telefono    in proveedor.telefono%type,
                               p_email       in proveedor.email%type,
                               p_direccion   in proveedor.direccion%type) is
  begin
    insert into proveedor(id_proveedor, nombre, telefono, email, direccion)
    values (p_id_proveedor, p_nombre, p_telefono, p_email, p_direccion);
    commit;
  ...
  procedure actualizar_proveedor(p_id_proveedor in proveedor.id_proveedor%type,
                                ...
                                p_direccion   in proveedor.direccion%type) is
  begin
    update proveedor set nombre = p_nombre, telefono = p_telefono,
  ...
  begin
    open p_cursor for select * from proveedor;
  end obtener_proveedores;

  function cur_productos_proveedor(p_id_proveedor in proveedor.id_proveedor%type) return sys_refcursor is
    rc sys_refcursor;
  begin
    open rc for select p.* from producto p where p.id_proveedor = p_id_proveedor order by p.nombre;
    return rc;
  end cur_productos_proveedor;

  function fn_contar_productos_proveedor(p_id_proveedor in proveedor.id_proveedor%type) return number is
    v_count number;
  begin
    select count(*) into v_count from producto where id_proveedor = p_id_proveedor;
    return v_count;
  ...
  begin
    open rc for
      ...
      from proveedor pr join producto p on p.id_proveedor = pr.id_proveedor
